...
/*ETIQUETAS UTILIZADAS EN LA CLASE*/
...
//@GetMapping: acepta solicitudes http de tipo get que permiten recuperar datos de usuario
//@PostMapping: acepta solicitudes http de tipo post que contienen un cuerpo de petición de tipo JSON o XML permitiendo insertar datos de usuario
//@DeleteMapping: acepta solicitudes http de tipo delete que permite eliminar información de usuario
//@PathVariable: permite asignar una variable de la ruta url a un objeto
//@RequestBody: indica que un parámetro del método debe estar vinculado al valor del cuerpo de la petición http
//@Valid: comprueba si la información que se le está pasando al método es correcta o no
...
import conexiona.pratices.noapractices.service.SAccount;
...
public class CAccount {
    ...
    public ResponseEntity<Void> addAccount(@RequestBody @Valid MAccount mAccount, UriComponentsBuilder builder){
        boolean flag = sAccount.insertAccount(mAccount);
        if(!flag){
            return new ResponseEntity<>(HttpStatus.CONFLICT);
        }else {
            HttpHeaders headers = new HttpHeaders();
            headers.setLocation(builder.path("/accountSelect/{accountId}").buildAndExpand(mAccount.getAccountId()).toUri());
    ...
    public ResponseEntity<MAccount> updateAccount(@RequestBody @Valid MAccount mAccount) {
        boolean flag = sAccount.updateAccount(mAccount);
        if (!flag){
            return new ResponseEntity<>(mAccount, HttpStatus.NOT_FOUND);
        }else {
            return new ResponseEntity<>(mAccount, HttpStatus.OK);
        }
    }
    ...
    public ResponseEntity<Void> deleteAccount(@PathVariable("accountId") String accountId) {
        boolean flag = sAccount.deleteAccount(accountId);
        if(!flag){
    ...
    @GetMapping("accountSelect/{accountId}")
    public ResponseEntity<MAccount> getAccountById(@PathVariable("accountId") String accountId) {
        MAccount mAccount = sAccount.selectAccountByAccountId(accountId);
        return new ResponseEntity<>(mAccount, HttpStatus.OK);
    }

    @GetMapping("/accountSelect")
    public ResponseEntity<List<MAccount>> getAllAccount(){
       List<MAccount> accountList = sAccount.selectAllAccount();
       return new ResponseEntity<>(accountList, HttpStatus.OK);
    }

   @GetMapping("/accountPageableSelect")
    public ResponseEntity<List<MAccount>> getPageableAccount(Pageable pageable){
        List<MAccount> accountList = sAccount.selectPageableAccount(pageable);
        return new ResponseEntity<>(accountList, HttpStatus.OK);
    }

    @GetMapping("/accountUserSelect/{accountId}")
    public ResponseEntity<List<MUser>> getUserByAccountId(@PathVariable("accountId") String accountId){
        List<MUser> userList = sAccount.selectUserbyAccountId(accountId);
        return new ResponseEntity<>(userList, HttpStatus.OK);
    }

    @GetMapping("/accountUserGroupSelect/{accountId}")
    public ResponseEntity<List<MUserGroup>> getUserGroupByAccountId(@PathVariable("accountId") String accountId){
        List<MUserGroup> userGroupList = sAccount.selectUserGroupbyAccountId(accountId);
