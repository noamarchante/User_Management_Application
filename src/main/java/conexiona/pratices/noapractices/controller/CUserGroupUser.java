...
import conexiona.pratices.noapractices.service.SUserGroupUser;
...
public class CUserGroupUser {
    ...
    public ResponseEntity<Void> addUserGroupUser(@RequestBody @Valid MUserGroupUser mUserGroupUser, UriComponentsBuilder builder) {
        ...
        if (!flag) {
            ...
            headers.setLocation(builder.path("/userGroupUserSelect/{userGroupUserId}").buildAndExpand(new UserGroupUserId(mUserGroupUser.getUserId(), mUserGroupUser.getUserGroupId())).toUri());
    ...
    public ResponseEntity<MUserGroupUser> updateUserGroupUser(@RequestBody @Valid MUserGroupUser mUserGroupUser) {
        boolean flag = sUserGroupUser.updateUserGroupUser(mUserGroupUser);
        if (!flag) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping("/userGroupUserDelete/{userId}/{userGroupId}")
    public ResponseEntity<Void> deleteUserGroupUser(@PathVariable("userId") String userId, @PathVariable("userGroupId") String userGroupId) {
        boolean flag = sUserGroupUser.deleteUserGroupUser(new UserGroupUserId(userId, userGroupId));
        if (!flag) {
    ...
    @GetMapping("userGroupUserSelect/{userId}/{userGroupId}")
    public ResponseEntity<MUserGroupUser> getUserGroupUserById(@PathVariable("userId") String userId, @PathVariable("userGroupId") String userGroupId ) {
        MUserGroupUser mUserGroupUser = sUserGroupUser.selectUserGroupUserByUserGroupUserId(new UserGroupUserId(userId, userGroupId));
        return new ResponseEntity<>(mUserGroupUser, HttpStatus.OK);
    }

    @GetMapping("/userGroupUserSelect")
    public ResponseEntity<List<MUserGroupUser>> getAllUserGroupUser() {
        List<MUserGroupUser> userGroupUserList = sUserGroupUser.selectAllUserGroupUser();
    ...
    public ResponseEntity<List<MUserGroupUser>> getPageableUserGroupUser(Pageable pageable){
        List<MUserGroupUser> userGroupUserList = sUserGroupUser.selectPageableUserGroupUser(pageable);
        return new ResponseEntity<>(userGroupUserList, HttpStatus.OK);
    }
}
